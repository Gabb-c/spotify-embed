...
describe('URL Parser', () => {
  it('sholud return a formated (track) embed URL', () => {
    ...
    expect(parsed).toBe(
      SpotifyEmbedUrl.TRACK + '5mslcDXYQbmOFL6s031NvN' + `?theme=${Themes.LIGHT}`
    );
  });
  ...
  it('sholud return a formated (artist) embed URL', () => {
    ...
    expect(parsed).toBe(
      SpotifyEmbedUrl.ARTIST + '10ZLWWtQcT81iBuVeGwlgO' + `?theme=${Themes.LIGHT}`
    );
  });
  ...
  it('sholud return a formated (album) embed URL', () => {
    ...
    expect(parsed).toBe(
      SpotifyEmbedUrl.ALBUM + '7zFWWsc1JWT3hOypzEMak4' + `?theme=${Themes.LIGHT}`
    );
  });
  ...
  it('sholud return a formated (playlist) embed URL', () => {
    ...
    expect(parsed).toBe(
      SpotifyEmbedUrl.PLAYLIST + '2NdDBIGHUCu977yW5iKWQY' + `?theme=${Themes.LIGHT}`
    );
  });
  ...
  it('sholud return a formated (episode) embed URL', () => {
    ...
    expect(parsed).toBe(
      SpotifyEmbedUrl.EPISODE + '5dpInH3tAoV8qFxURbXAEv' + `?theme=${Themes.LIGHT}`
    );
  });
